...
def collapse_rows_to_wide(df, groupby_cols, agg_cols, delimiter=";"):
    ...
    if (
        "ON_STUDY_DX.SUPPMH_QVAL_MHIDRF_LOG" in df.columns
        and "ON_STUDY_DX.SUPPMH_QVAL_MHIDRF_LOG" in agg_cols
    ):
        # perform aggregation of this column first to avoid issues with large text
        df["ON_STUDY_DX.SUPPMH_QVAL_MHIDRF_LOG"] = df.groupby(groupby_cols)[
            "ON_STUDY_DX.SUPPMH_QVAL_MHIDRF_LOG"
        ].transform(
            lambda x: (
                delimiter.join(
                    [
        ...
        df["ON_STUDY_DX.SUPPMH_QVAL_MHIDRF_LOG"] = df[
            "ON_STUDY_DX.SUPPMH_QVAL_MHIDRF_LOG"
        ].str.strip(delimiter)
...
def save_to_workbook(workbook_name: str, df: pd.DataFrame, sheet_name: str, mode: str, reorder=None):
    ...
    if mode == "a":
        with pd.ExcelWriter(
            workbook_name, engine="openpyxl", mode=mode, if_sheet_exists="replace"
        ) as writer:
            df.fillna("").to_excel(
...
def substudy_sheet(
    ...
    if other:
        ...
        ss_cols = (
            int_df[(int_df["substudy"] == substudy) | (int_df["substudy"] == "ALL")][
                "label"
            ]
            .unique()
            .tolist()
    ...
    if other:
        ...
        primary_disease_groups = [
            item for sublist in PRIMARY_GROUPS.values() for item in sublist
        ]
        cog_prim_dis_subset_df = cog_subset_df[
            ~cog_subset_df["Primary Disease Group/Sub-study"].isin(
                primary_disease_groups
            )
        ]
    else:
    ...
    merged_df = pd.merge(
        cog_prim_dis_subset_df, igm_merged_df, on="Subject ID", how="left"
    ).drop_duplicates()
    ...
    if substudy != "CNS" or other:
        igm_methylation_cols = int_df[
            (int_df["type"] == "igm.methylation") & (int_df["label"] != "Subject ID")
        ]["label"].tolist()
        merged_df = merged_df.drop(columns=igm_methylation_cols, errors="ignore")

    # get col order from int_df
    col_order = (
        int_df[int_df["label"].isin(merged_df.columns)]
        .sort_values("integrated_sheet_order")["label"]
        .drop_duplicates()
        .to_list()
    )
    ...
    if other:
        save_to_workbook(
            workbook_name, merged_df, f"COG IGM Integrated - Other", "a", reorder=True
        )
    else:
        save_to_workbook(
            workbook_name,
            merged_df,
            f"COG IGM Integrated - {substudy}",
            "a",
            reorder=True,
        )
...
def cog_igm_integrate(
    ...
    for k, v in integration_files.items():
        if (k == "COG") & (v != ""):
            if os.path.exists(v):
                ...
                logger.error(
                    f"Parsed file {v} does not exist at path, not integrating in COG/IGM integration."
                )
        elif v != "":
            if os.path.exists(v):
                IGM = True
                ...
                logger.error(
                    f"Parsed file {v} does not exist at path, will not be included for COG/IGM integration."
                )
                rem.append(k)
    ...
    if len(integration_files) < 2:
        logger.info(
            "Missing COG or IGM files to integrate, will not perform integration."
        )
        return None
    ...
    for k, v in integration_files.items():
        if k == "COG":
            ...
            temp_df = pd.read_csv(v, sep="\t", skiprows=[1, 2], low_memory=False).map(
                fix_encoding_issues
            )
        else:
            temp_df = pd.read_csv(v, sep="\t", low_memory=False).map(
                fix_encoding_issues
            )
        ...
        if not all(col in temp_df.columns for col in cols) and k == "COG":
            ...
            logger.warning(
                f"Not all columns specified in integration mapping found in {k} dataframe. Columns not found: {missing_cols}"
            )
        ...
        # check if IGM file has no data to parse
            ...
            logger.info(
                f"Not all columns specified in integration mapping found in {k} dataframe. Source data may not have had variants/alterations identified for these participants. Skipping {k} dataframe for integration."
            )
        ...
        if len(multi_field_labels) > 0:
            for m in multi_field_labels:
                mult_cols = int_df[
                    (int_df["source"] == k)
                    & (int_df["label"] == m)
                    & (int_df["data_elements"].isin(parse_cols))
                ]["data_elements"].to_list()

                # create new col with concatenated values, excluding nans
                temp_df[m] = temp_df[mult_cols].apply(
        ...
        to_transpose = int_df[
            (int_df["source"] == k) & (int_df["modifier"] == "transpose")
        ]["data_elements"].to_list()

        if len(to_transpose) > 0:
            if k == "COG":
                # collapse rows to wide format
                temp_df = collapse_rows_to_wide(
                    temp_df, groupby_cols="upi", agg_cols=to_transpose, delimiter=";"
                )
        ...
        for col in current_cols:
            ...
            label_match = int_df[
                (int_df["source"] == k) & (int_df["data_elements"] == col)
            ]["label"]
    ...
    if len(df_list) > 0:
        ...
        if len(igm_dfs) > 1:
            ...
            col_order = (
                int_df[int_df["label"].isin(igm_merged_df.columns)]
                .sort_values("igm_sheet_order")["label"]
                .drop_duplicates()
                .to_list()
            )
            ...
            save_to_workbook(
                workbook_name, igm_merged_df, sheet_names[igm_dfs[0][0]], "w"
            )

            for df in igm_dfs[1:]:
                # order cols in order from order in int_df
                col_order = (
                    int_df[int_df["label"].isin(df[1].columns)]
                    .sort_values("igm_sheet_order")["label"]
                    .drop_duplicates()
                    .to_list()
                )
                ...
                shared_cols = set(igm_merged_df.columns).intersection(df[1].columns) - {
                    "Subject ID"
                }
                df[1] = df[1].drop(columns=shared_cols, errors="ignore")

                # merge to merged_df
                igm_merged_df = pd.merge(
                    igm_merged_df, df[1], on="Subject ID", how="outer"
                )
            ...
            # order cols in order from order in int_df
            col_order = (
                int_df[int_df["label"].isin(igm_dfs[0][1].columns)]
                .sort_values("igm_sheet_order")["label"]
                .drop_duplicates()
                .to_list()
            )
        ...
        substudies = (
            int_df[(int_df["substudy"] != "ALL") & (int_df["substudy"].notnull())][
                "substudy"
            ]
            .unique()
            .tolist()
